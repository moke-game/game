...
func (rm *RoomMgr) LoadOrCreateRoom(roomId string, creator riface.RoomCreator, allowCreat bool) (riface.IRoom, error) {
	if r, ok := rm.rooms.Load(roomId); !ok {
		if roomId != "10000" && !allowCreat {
			return nil, errors.New("RoomNotFound")
		}
		cr, err := creator()
		if err != nil {
			return nil, err
