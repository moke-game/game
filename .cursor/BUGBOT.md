...
- `GrpcModule` / `HttpModule` / `AllModule` have no auth provider — pair `modules.Platform` / `AuthAllModule` (aggregate) or `modules.PlatformClients` / `AuthMiddlewareModule` (thin) in `main`, never both with a stub.
